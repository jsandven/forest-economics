...
age <- 1:150
df <- data.frame(age)
df$CAI <- vol_spruce_gizachew_2012(SI = si, spacing = 4, age = df$age, method = "CAI")
df$volume <- vol_spruce_gizachew_2012(SI = si, spacing = 4, age = df$age, method = "Volume")
...
which.min(abs(df$return_25 - df$vol_retur))
which.min(abs(df$return_35 - df$vol_retur))
which.min(abs(df$return_45 - df$vol_retur))
which.min(abs(ifelse(df$age < 10,100,df$CAI - df$MAI)))
```
